# Teknoloji Kartları — her parça ne işe yarıyor

> **Bu belge Barış'ın okuması ve çalışması için yazıldı.**
> ⛔ Ajan bunu okumaz; ajanın uyacağı kurallar `skills/yeni-proje/dosyalar/docs/standards/`
> altındadır. Buradaki amaç kural koymak değil, **anlamak**.

**Son güncelleme:** 2026-09-06

---

## Bu belge nereden geldi

Kaynağı, bakım ve iş emri projesi için yazılan `proje-teknoloji-ve-plan.md`.
O belgenin **her projede geçerli olan** bölümleri buraya alındı; o projeye özel
olan kısımlar (ödevin 12 maddesi, eşleme tabloları, yapım planı) tam hâliyle
`ornek-proje-bakim-is-emri/proje-teknoloji-ve-plan.md` içinde duruyor.

Örnekler bakım/iş emri sisteminden alınmıştır ve **öyle bırakılmıştır** —
soyut örnek yerine gerçek bir sistem üzerinden anlatmak öğrenmeyi kolaylaştırır.
...
# BÖLÜM Ö — Projeye başlarken: elimizde ne var, ne seçiyoruz

Bu bölüm 2026-09-06'da eklendi. Diğer bölümler *"teknoloji ne yapar"* sorusunu
cevaplıyor; bu bölüm *"hangi durumda hangisini kullanırız"* sorusunu.

## Ö.1 React ile Next.js farkı

**React** bir **kütüphane / library**. Tek işi var: ekranı bileşenlere bölüp
çizmek. Yönlendirme (routing) yok, sunucu tarafı yok, derleme kurulumu yok —
onları sen eklersin.

**Next.js** bir **çatı / framework**. İçinde React var, üstüne şunları hazır
getirir: sayfa yönlendirme, sunucuda çalışma, API yazma yeri, görsel
optimizasyonu, derleme.

### ⛔ "Router" kelimesi iki farklı şey için kullanılıyor

| Terim | Ne yapar |
|---|---|
| **Next App Router** | **Sayfa** yönlendirme: `/randevular` adresine gidince hangi ekran açılır |
| **Express router** | **API ucu** tanımlama: `POST /api/randevular` isteğini hangi fonksiyon karşılar |

Express'in yaptığı işin Next'teki karşılığı **Route Handler** —
`app/api/randevular/route.ts` dosyası. App Router değil. Aynı kelime, farklı iş.

### React tek başına ne zaman yeter

| | React (+ Vite + React Router) | Next.js |
|---|---|---|
| Yönlendirme | Elle kurarsın | Hazır |
| Arama motoru içeriği görebilir mi | ⛔ Hayır | ✅ Evet |
| Gizli anahtar saklama | ⛔ Zor — her şey tarayıcıda | ✅ Sunucu tarafı var |
| httpOnly oturum çerezi | ⛔ Sunucusuz zor | ✅ Kolay |

React tek başına yalnızca şu durumda mantıklı: **giriş arkasında bir panel**,
arama motoru gerekmiyor ve barındırmayı başkası yapıyor. Onun dışında Next'in
maliyeti neredeyse sıfır, ondan geri dönmek ise pahalı.

## Ö.2 ⛔ Next.js backend'dir — ama veritabanı DEĞİLDİR

En sık karışan yer burası. *"Next.js mi Supabase mi"* diye bir seçim yoktur:

| Ne | Next.js sağlar mı |
|---|---|
| Arayüz | ✅ |
| API uçları (backend kodu) | ✅ Route Handler |
| İş mantığı · servis · repository | ✅ |
| **Veritabanı** | ⛔ **Hayır — ayrı bir şeydir, her zaman lazımdır** |

Veritabanını nereden alacağın **ayrı bir karardır**:

| Seçenek | Ne verir | Ne zaman |
|---|---|---|
| **Neon** | Yönetilen PostgreSQL + **dallanma** (her PR kendi veritabanı kopyasıyla test edilir) | Varsayılan — giriş için Auth.js kullanılıyorsa |
| **Supabase** | PostgreSQL + hazır giriş + depolama + gerçek zamanlı, tek pakette | Hızlı başlangıç isteniyorsa; ama giriş modeline bağlanırsın |
| **Docker'da PostgreSQL** | Kendi bilgisayarında, ücretsiz | Yerel geliştirme · kurum sunucusu |
| **Kurumun PostgreSQL'i** | Kurum verir | Belediye/kurum projeleri |

## Ö.3 NestJS ne zaman gerekir — "hiçbir şey yoksa" DEĞİL

⛔ Yaygın yanlış: *"hiçbir şey verilmediyse Next + Nest yaparız."* Kural bu
değil. Nest'i gerektiren **dört soru** vardır; biri bile "evet" ise Nest gelir:

1. API'yi kendi arayüzünden **başkası** da tüketecek mi? (mobil, başka sistem)
2. Kimse ekranı açmasa da **kendiliğinden** çalışacak iş var mı? (gece taraması,
   zamanlanmış hatırlatma, webhook karşılama)
3. Çok modüllü yapı ve **DI yaşam döngüsü** gerekiyor mu?
4. Kod **kurumun kendi sunucusunda** mı çalışacak?

Belediye projelerinde genelde **dördüncüsü** "evet" olduğu için Nest çıkar —
"hiçbir şey yok" olduğu için değil.

## Ö.4 Elimizde ne hazır — sekiz senaryo

```
Kurum / durum ne veriyor?
│
├─ 1. HİÇBİR ŞEY (kendi projem)
│     → hepsi bizde: veritabanı seç → şema yaz → migrate → API → arayüz
│
├─ 2. Sadece veritabanında YER ayrılmış (boş şema)
│     → tabloları BİZ açarız: schema.prisma → prisma migrate dev
│
├─ 3. Veritabanı + TABLOLAR hazır
│     → prisma db pull (şemayı koda çeker) → API + arayüz bizde
│     ⚠️ Tablo adları kurumun kuralına uyar; bizim isimlendirme kuralımız GEÇMEZ
│
├─ 4. Veritabanı + API hazır
│     → sadece arayüz. ⛔ Prisma GEREKMEZ, repository katmanı BİZDE YOK
│     → bize lazım olan: API sözleşmesi (contract)
│
├─ 5. Kimlik kurumdan geliyor (LDAP · Active Directory · kurum SSO)
│     → giriş ekranı yazmıyoruz, kurumun sistemine bağlanıyoruz
│     → Auth.js yerine entegrasyon; oturum ve rol kurumdan gelir
│
├─ 6. Mevcut sistemle ENTEGRASYON isteniyor (eski ERP · e-Devlet · başka birim)
│     → kendi veritabanımızın YANINDA başkasının API'sini de tüketiyoruz
│     → dış çağrılar sunucu tarafında yapılır ve önbelleklenir
│
├─ 7. Çalışan projeye EKLEME (brownfield)
│     → kurulum yok, mevcut mimariye uyum var
│     → önce oku ve haritasını çıkar, sonra dokun
│
└─ 8. Veri var ama BAŞKA BİÇİMDE (Excel · Access · eski sistem dökümü)
      → önce taşıma/aktarım işi, sonra proje
```

⚠️ **Senaryolar birleşebilir.** Belediyede en sık görülen bileşim: 2 + 5 + 6 —
boş şema verilir, kimlik kurumdan gelir, bir de eski sistemle konuşulur.

## Ö.5 Kurum projesi mi, kendi projem mi — neyi değiştirir

| | Kendi projem | Kurum projesi |
|---|---|---|
| Stack | Sen seçersin (kit varsayılanı) | ⛔ **Şartname/kurum dayatır** — önce sor, sonra karar ver |
| Yayına alma | Sen çıkarırsın (Vercel + Neon) | ⛔ **Sen çıkarmazsın** — DevOps'a teslim paketi hazırlarsın |
| Domain, DNS, SSL | Senin işin | ⛔ Hiç açma |
| İsimlendirme | Kitin kuralı | Kurumun kuralı kazanır |

⭐ **Gerçek örnek:** bakım/iş emri ödevinde kurum **C# / .NET / ASP.NET Core
Web API / React** istemişti. Stack değiştirme kararının gerekçesi
`ornek-proje-bakim-is-emri/proje-teknoloji-ve-plan.md` → *"Giriş — neden stack
değişti"* bölümündedir. Bu, "kurum stack dayatır" senaryosunun canlı örneğidir.

## Ö.6 Kullanılan dış servisler — her biri hangi sorunu çözüyor

| Servis | Ne yapar | Olmasaydı |
|---|---|---|
| **Vercel** | Next.js'i barındırır (hosting), CDN'den dağıtır, her PR'a önizleme adresi verir | Sunucu kiralar, Docker kurar, HTTPS ayarlar, güncellerdin |
| **Neon** | Yönetilen PostgreSQL; yedek alır, **dallanma** yapar | Veritabanını sen kurar, yedekler, yamalardın |
| **Cloudflare Turnstile** | Giriş gerektirmeyen formlarda bot ayırma | Formları botlar doldururdu |
| **Resend** | Uygulamanın e-posta göndermesi (doğrulama kodu, şifre sıfırlama) | Kendi e-posta sunucun spam'e düşerdi |
| **Google Cloud** | **Google ile giriş** (OAuth) — `benim-belediyem` projesinde bunun için kuruldu | Kullanıcı ayrı şifre oluşturmak zorunda kalırdı |
| **Vercel Blob** | Kullanıcıların yüklediği dosyalar | Dosyalar sunucuda birikir, her deploy'da silinirdi |
| **Sentry** | Canlıdaki hataları yakalar ve bildirir | Hatayı ancak kullanıcı şikâyet edince duyardın |

## Ö.7 Bir isteğin tam yolu — ORM nerede durur
...
dosyadır (`randevu.repository.ts`); ORM onun **kullandığı** pakettir. Şemayı
*"Repository (Prisma)"* diye yazmak yaygın bir hatadır ve katman sınırını
görünmez kılar.

| # | Durak | Ne yapar | Ne YAPMAZ |
|---|---|---|---|
| 1 | **İstemci** | İsteği gönderir | İş kuralı bilmez |
| 2 | **API katmanı** | Zod ile doğrular · kimliği çözer · servisi çağırır · HTTP'ye çevirir | ⛔ İş kuralı yazılmaz |
| 3 | **Servis** | Kuralları uygular: çalışma saati · aynı gün ikinci kayıt · slot dolu mu | ⛔ `req`/`res` tanımaz |
| 4 | **Repository** | ORM'e ne isteneceğini söyler | ⛔ Tek bir `if` bile bulunmaz |
| 5 | **ORM (Prisma)** | Çağrıyı SQL'e, dönen satırı nesneye çevirir | Karar vermez |
| 6 | **PostgreSQL** | SQL'i çalıştırır | — |

### ⛔ İki yaygın yanlış anlama

**1. "ORM gelen isteği doğrular."** Hayır:

| Ne doğrulanır | Ne zaman | Kim |
|---|---|---|
| Gelen **isteğin** içeriği | Çalışma anında, her istekte | **Zod** |
| **Kodun** tabloyla uyumu | Derleme anında, sen yazarken | **TypeScript** |

**2. "Gerekirse migration istek sırasında çalışır."** ⛔ **Asla.**
`prisma migrate dev` kendi bilgisayarında, `prisma migrate deploy` yayına
alırken **bir kez**. Ani yükte 20.000 isteğin veritabanı yapısını değiştirmeye
kalkması felaket olurdu — yapı önceden hazırdır.
