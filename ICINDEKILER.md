...
| `hooks/kural-log.mjs` | Ölçüm kancası: `InstructionsLoaded` (hangi kural, neden, tetikleyen) ve `PostToolUse(Read|Bash)` (hangi standart okundu) → `~/.claude/proje-kiti/log/<proje>.jsonl`. Karar vermez, engellemez |
...
| `calisilacak-konular.md` | Senin öğrendiklerin — tek defter, her projede aynı, kite geri döner |
